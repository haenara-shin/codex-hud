...
- **app-server / Codex 플러그인 경로 지원.** Codex 0.140+를 app-server(예: Claude Code codex 플러그인)로 돌리면 rate limit이 rollout 로그에 안 쓰이고 `~/.codex/logs_2.sqlite`에 기록됩니다. codex-hud가 거기서 최신 스냅샷을 읽습니다(로컬, 네트워크/인증 없음). rollout과 신선도로 병합. 플러그인으로만 Codex를 쓰던 사용자의 "No Codex sessions found" 문제 해결.
- **reasoning effort**를 모델 배지에 표시(예: `gpt-5.5·xhigh`), `~/.codex/config.toml`에서 읽음.
- primary 윈도우 **`5h Usage` 라벨**(윈도우 길이에서 파생), Codex `/status`와 일치.
- **쿼터를 "% left"로 표시**, 바는 남은 양만큼 채움 — 예: `5h Usage ██████████ 99% left (resets 19:38 · 3h 58m)` — Codex `/status`와 일치. (Context 바는 used% 유지.)
- 새 app-server 소스는 Node >= 22.5 또는 `sqlite3` CLI 필요; 없으면 graceful degrade.
